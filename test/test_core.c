...
TEST(test_core_start_sequence)
{
    ...
    /* OFF -> PRESTART */
    ecu_outputs_t out = ecu_step(&ecu, &in, 0.01f);
    ASSERT_EQ(out.state, ECU_STATE_SPINUP);  /* prestart immediately transitions */

    /* SPINUP — starter should be on */
    in.rpm = 5000.0f;
    out = ecu_step(&ecu, &in, 0.1f);
    ASSERT(out.starter_on);

    /* Get to ignition RPM */
    in.rpm = 15000.0f;  /* > idle * 0.3 */
    out = ecu_step(&ecu, &in, 0.1f);
...
TEST(test_core_ignition_to_ramp)
{
    ...
    /* Get to IGNITION state */
    ecu_step(&ecu, &in, 0.01f);
    in.rpm = 15000.0f;
    ecu_step(&ecu, &in, 0.1f);

    /* In IGNITION — provide EGT rise */
    in.egt = 200.0f;  /* above egt_start_min (150) */
...
TEST(test_core_fault_on_timeout)
{
    ...
    ecu_inputs_t in = { .rpm = 0, .egt = 25.0f, .throttle = 50.0f };
    ecu_step(&ecu, &in, 0.01f);  /* OFF -> PRESTART -> SPINUP */

    /* RPM stays low, should timeout */
    in.rpm = 100.0f;
    for (int i = 0; i < 20; i++) {
        ecu_step(&ecu, &in, 0.01f);
    }
